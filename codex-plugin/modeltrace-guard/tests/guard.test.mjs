...
test('new tasks default to 20–60 work tools without overwriting saved custom intervals', async (t) => {
  assert.equal(DEFAULTS.toolMin, 20); assert.equal(DEFAULTS.toolMax, 60);
  assert.equal(validateConfig({}).toolMin, 20); assert.equal(validateConfig({}).toolMax, 60);
  const directory = await fixture(t);
  const started = await run(['start', ...flags(directory)], {});
  assert.equal(started.frequency.toolMin, 20); assert.equal(started.frequency.toolMax, 60);
...
test('never-enabled legacy hook records pick up new defaults but restarted tasks keep saved intervals', async (t) => {
  ...
  assert.equal(first.frequency.toolMin, 20); assert.equal(first.frequency.toolMax, 60);
  ...
  assert.equal(first2.frequency.toolMin, 20); assert.equal(first2.frequency.toolMax, 60);
  ...
  assert.equal(first3.frequency.toolMin, 20); assert.equal(first3.frequency.toolMax, 60);
});
